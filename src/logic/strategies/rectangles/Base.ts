      ...
      return this.cells()
         .map(origin => {
            ...
            return pairs.map(pair => {
               ...
               return withPair
                  .filter(it => origin.col === it.col && origin.row < it.row)
                  .map(inCol => {
                     // Search for cells in the same row with these candidates
                     return withPair
                        .filter(it => origin.row === it.row && origin.col < it.col)
                        .map(inRow => {
                           ...
                           const withTwo = corners
                              ...
                              .filter((c1, i1, a) => a.some((c2, i2) => i1 !== i2 && (c1.row === c2.row || c1.col === c2.row || ninthAt(c1) === ninthAt(c2))))
